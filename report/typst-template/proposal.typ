...
\
- Techniques like UMAP, t-SNE, and PHATE treat brain activity as isolated snapshots, failing to generate the continuous temporal flow of information. A framework, Brain-dynamic Convolutional-Network-based Embedding (BCNE), has been proposed to visualize and understand the fMRI and EEG data #cite(<refBCNE>).
...
#[
  #show figure: set align(left) 
   + #goal("Simulate continuous EEG data using UnfoldSim.jl.") <goal1>  
  + #goal("Implement dimensionality reduction and visualization techniques on simulated continuous EEG data in Python.") <goal2> 
...
== Approach <approach>
*Phase 1 *
- Data Simulation: Use UnfoldSim.jl, a Julia package for simulating  time series data #cite(<refunfoldsim>) focusing on EEG and event-related potentials (ERPs). 
- Simulate EEG data with 32 channels using the predef_eeg function, generating realistic signals based on 4 ERP components: N170, P300, N1/N400, and P1.
- Simulate 5 trials for 2 conditions and deactivate components one by one to see the differences and effects on the data.
- Expected geometry after dimensionality reduction (PHATE, TPHATE, BCNE): Branch trajectory structure for 4 different components such as P300, N170, P1 and N1 starting from initial time point. There might be space-related differences as there are 32 channels for each ERP component.
